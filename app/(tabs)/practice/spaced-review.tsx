...
export default function SpacedReviewScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const load = async () => {
        try {
          if (!user) return;
          const cards = await spacedRepetitionService.getDueCards(user.id, 20);
          if (cancelled) return;
          setQuestions(cards);
  ...
  const handleAnswer = async (questionId: string, selectedOptionIds: string[], isCorrect: boolean) => {
    try {
      ...
      await questionService.recordAttempt(user.id, questionId, selectedOptionIds, isCorrect, timeSpent, 'spaced_review');
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={styles.center}>
          ...
          <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
            No cards due for review. Keep practicing to build your deck.
          </Text>
          <TouchableOpacity
        ...
        {answered && (
          ...
          >
            <Text style={styles.nextText}>
              {currentIndex < questions.length - 1 ? 'Next Card' : 'Finish'}
            </Text>
