...
export const questionService = {
  ...
  async getMockExamQuestions(): Promise<Question[]> {
    ...
    // Single query: pick N random questions per category using ROW_NUMBER
    const rows = await db.getAllAsync<any>(
      `SELECT q.* FROM (
         SELECT q.*, ROW_NUMBER() OVER (PARTITION BY t.category ORDER BY RANDOM()) as rn
         FROM questions q
         JOIN topics t ON q.topic_id = t.id
       ) q WHERE q.rn <= ?
       ORDER BY RANDOM()
       LIMIT ?`,
      [questionsPerCategory, APP_CONFIG.MOCK_EXAM_QUESTIONS]
    );
  ...
  async getWrongAnswers(userId: string): Promise<Question[]> {
    const db = await getDatabase();
